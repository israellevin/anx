...
LOG_DIR_NAME = os.environ.get('ANX_LOG_DIR', './')
LOG_FILE_NAME = os.environ.get('ANX_LOG_FILE', 'anx.log')
FORMAT = os.environ.get('ANX_LOG_FMT', '%(asctime)s %(levelname).3s: %(message)s - %(name)s +%(lineno)03d')
DATE_FORMAT = os.environ.get('ANX_LOG_DATE_FMT', '%Y-%m-%d %H:%M:%S')
LEVEL = os.environ.get('ANX_LOG_LEVEL', logging.DEBUG)
